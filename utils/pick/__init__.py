...
class Picker:
    ...
    options_color: int = curses.COLOR_YELLOW
    title_color: int = curses.COLOR_GREEN
    ...
    def __post_init__(self):
        ...
        if self.multiselect and self.min_selection_count > len(self.options):
            raise ValueError('min_selection_count is bigger than the available options,'
    ...
    def draw(self, screen):
        ...
        titledraw = 0  # the number of lines we drew in the title
        ...
        for line in lines_to_draw:
            if type(line) is tuple:
                ...
                if len(self.get_title_lines()) > titledraw:
                    screen.addnstr(y, x, line, max_x-2, curses.color_pair(1))
                    titledraw += 1
                else:
                    screen.addnstr(y, x, line, max_x-2, curses.color_pair(2))
            y += 1
    ...
    def config_curses(self):
        try:
            ...
            curses.init_pair(1, self.title_color, curses.COLOR_BLACK)
            curses.init_pair(2, self.options_color, curses.COLOR_BLACK)
        except curses.error:
            # Curses failed to initialize color support, eg. when TERM=vt100
            print_exception('Failed to initialize color support')
            curses.initscr()
